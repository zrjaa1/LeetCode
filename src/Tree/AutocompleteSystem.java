...
public class AutocompleteSystem {
    ...
    private void insert(String sentence, int time) {
        ...
        for (char ch: sentence.toCharArray()) {
            ...
            cur = cur.children[idx];
            updateTop3(cur.top3, sentence, time);
        }
    }

    private void updateTop3(List<Pair> top3, String sentence, int time) {
        boolean exist = false;
        for (Pair pair: top3) {
        ...
        if (!exist) {
            top3.add(new Pair(sentence, time));
        }
    ...
    private List<String> getTop3(TrieNode cur) {
        ...
        for (int i = 0; i < cur.top3.size(); i++) {
            if (i == 3) {
                break;
            }
            res.add(cur.top3.get(i).sentence);
        }
        return res;
    }
