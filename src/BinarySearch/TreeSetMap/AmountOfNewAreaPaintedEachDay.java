...
public class AmountOfNewAreaPaintedEachDay {
    public int[] amountPainted(int[][] paint) {
        TreeMap<Integer, Integer> treeMap = new TreeMap<>(); // key - start, value - end
        ...
        for (int i = 0; i < paint.length; i++) {
            ...
            if (leftEntry != null && leftEntry.getValue() >= paint[i][0]) { // if there is an interval that starts before current one and has overlap with us.
            ...
            // deal with following overlaps
            while (rightEntry != null && rightEntry.getKey() <= paint[i][1]) {
