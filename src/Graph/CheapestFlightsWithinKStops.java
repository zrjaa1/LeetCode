...
public class CheapestFlightsWithinKStops {
    ...
    /**
     * Solution 2: Dijkstra. No need to avoid circle as long as there is no negative circle and the path guarantee to exist(otherwise it might go into indefinite loops unless we use a set to contain points that their shortest path has been determined)
    ...
    public int findCheapestPrice(int n, int[][] flights, int src, int dst, int K) {
        ...
        while (!minHeap.isEmpty()) { // O(V)
            ...
            if (cur.name == dst) {
                System.out.println(cur.path); // 用来记录当前path，与本题无关，算是follow up
                return cur.distance;
            }
            if (cur.steps >= K + 1) {
                continue;
     ...
     * Bellamn-Ford can be used to find the shortest distances from a single source to to ANY destination in the graph.
     * Bellman-Ford is also a greedy algorithm, but less greedy than Dijkstra. Everytime it tries to relax a set of destinations, instead of only 1.
     * Usually, Bellman-Ford takes O(V) times to make sure the shortest path is calculated. In this problem, we only run K times.
     * O(T) = O(K * E), O(S) = O(V)，记住Bellman-Ford 是O（V*E) while Dijkstra is O(E + VlogV)
     */
