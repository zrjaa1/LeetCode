package DP;

import java.util.ArrayList;
import java.util.List;

/**
 * 115. Distinct Subsequences: https://leetcode.com/problems/distinct-subsequences/
 * Hard
 *
 * Given two strings s and t, return the number of distinct subsequences of s which equals t.
 *
 * A string's subsequence is a new string formed from the original string by deleting some (can be none) of the characters without disturbing the remaining characters' relative positions. (i.e., "ACE" is a subsequence of "ABCDE" while "AEC" is not).
 *
 * The test cases are generated so that the answer fits on a 32-bit signed integer.
 *
 *
 *
 * Example 1:
 *
 * Input: s = "rabbbit", t = "rabbit"
 * Output: 3
 * Explanation:
 * As shown below, there are 3 ways you can generate "rabbit" from S.
 * rabbbit
 * rabbbit
 * rabbbit
 * Example 2:
 *
 * Input: s = "babgbag", t = "bag"
 * Output: 5
 * Explanation:
 * As shown below, there are 5 ways you can generate "bag" from S.
 * babgbag
 * babgbag
 * babgbag
 * babgbag
 * babgbag
 *
 *
 * Constraints:
 *
 * 1 <= s.length, t.length <= 1000
 * s and t consist of English letters.
 */

/**
 * DP[i][j]: how many subsequence t[0, j] do we have in string s[0, i]. If there is match, we can choose to pick it or not.
 */
public class DistinctSubsequences {
    public int numDistinct(String s, String t) {
        int lenS = s.length(), lenT = t.length();
        int[][] dp = new int[lenS][lenT];

        for (int i = 0; i < lenS; i++) {
            for (int j = 0; j < lenT; j++) {
                if (i == 0 && j == 0) {
                    dp[i][j] = s.charAt(0) == t.charAt(0) ? 1 : 0;
                }
                else if (i == 0 && j != 0) {
                    dp[i][j] = 0;
                } else if (j == 0 && i != 0) {
                    dp[i][j] = howManyCharInString(s.substring(0, i + 1), t.charAt(0));
                } else { // i != 0 && j != 0
                    char ss = s.charAt(i);
                    char tt = t.charAt(j);
                    if (ss != tt) {
                        dp[i][j] = dp[i - 1][j];
                    } else { // ss == tt
                        dp[i][j] = dp[i - 1][j] + dp[i - 1][j - 1];
                    }
                }
            }
        }

        return dp[lenS - 1][lenT - 1];
    }

    private int howManyCharInString(String s, char t) {
        int res = 0;
        for (char c : s.toCharArray()) {
            if (c == t) {
                res++;
            }
        }

        return res;
    }

    public static void main(String[] args) {
        DistinctSubsequences tester = new DistinctSubsequences();
        String s = "babgbag";
        String t = "bag";
        int res = tester.numDistinct(s, t);
        System.out.println(res);
    }
}
