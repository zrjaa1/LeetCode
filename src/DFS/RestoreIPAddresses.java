...
/**
 * DFS. When to branch: if the current value is a valid value
 */
public class RestoreIPAddresses {
    ...
    private void dfs(String s, int idx, int num, StringBuilder path, List<String> res) {
        ...
        int len = path.length();
        for (int i = 0; i < 3; i++) { // for loop继续进行，是一种branch，而加'.'进入下一层dfs则是另一种branch
            ...
            if (val == 0) { // avoid case '00'.  only one 0 can be present
                break;
            }
        }
    }
