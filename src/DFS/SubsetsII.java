...
/**
 * A good example of picking first n consecutive elements.
 * 1. remove前n个的写法：其实是直接加到底，然后每次只remove 1个，先remove还是先pick其实没有关系，因为会set back
 * 2. pick前n个的写法，每次pick 1个，进dfs，然后每次remove到底 。
 */
public class SubsetsII {
    ...
    private void dfs(int[] nums, int idx, List<Integer> cur, List<List<Integer>> res) {
        ...
        for (int i = idx; i < nextIdx; i++) { // set back
