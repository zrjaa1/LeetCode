...
/**
 * DFS with pruning(idxS, idxP).
 * Base case: when idxP == lenP instead of idxS == lenS.
 */
...
public class RegularExpressionMatching {
    ...
    private static boolean dfsWithPruning(String s, String p, int idxS, int idxP, Boolean[][] mem) {
        ...
        if (idxP == lenP - 1 || p.charAt(idxP + 1) != '*') { // look at next char, if it's not *, we expect a match
            if (idxS < lenS && match(s, p, idxS, idxP)) { // our base case doesn't cover idxS == lenS
            ...
            int i = idxS - 1;
            while(i < lenS && (i == idxS - 1 || match(s, p, i, idxP))) { // match 0 or more elements
